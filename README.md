...
PCのブラウザには「ホーム画面に追加」が無いため、まずリンクをスマホへ渡す。

**QRで渡す（推奨）**
`/admin` にログイン →「QR用に鍵を表示」を押すとアドレスバーに鍵つきURLが戻る
→ ブラウザのメニューから「QRコードを作成」→ スマホで読み取る
→ 読み取った先で共有メニュー →「ホーム画面に追加」
鍵は3分で自動的にURLから消える。

**コピーして渡す**
「リンクをコピー」を押し、LINEのKeepメモやメールで自分に送ってスマホで開く。

以後はホーム画面のアイコンをタップするだけで、パスワード入力なしに開ける。
ホーム画面のリンクには鍵が入ったままなので、スマホ側の保存領域が
消えても（iOSは7日でクリアされることがある）タップすれば復帰する。

鍵はクエリ（`?k=`）ではなくフラグメント（`#k=`）に置いている。
フラグメントはサーバーに送信されないため、Vercelのアクセスログに鍵が残らない。
ページを開いた時点でアドレスバーからも消し、端末内に保存する。

**このリンクは鍵そのものなので、他人に渡さないこと。**
