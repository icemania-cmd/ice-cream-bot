...
- 原文が「298円（税込321.84円）」の形（コンビニに多い）のときは、298円は税抜。「価格：298円（税込321.84円）」とそのまま書く。「298円（税込）」としない（税表記の誤りになる）
- 販売エリアは原文に明記がある場合のみ「販売エリア＝全国」の形で書く
- 特定の店舗限定なら「セブン‐イレブン限定」のように明記する
- 「!!」は使ってよい。「→」「・」も可
- 商品名は product_name に入れたものと同じ表記を投稿文にも使う（言い換え・省略・別名の合成をしない）
- URL は入れない。ハッシュタグは付けない
- 絵文字は使ってもよいが最大2個まで。末尾に商品に合うものを1つ添える程度にとどめる
...
【禁止】書かれていない情報を足すこと、URL、ハッシュタグ、絵文字を3個以上使うこと
