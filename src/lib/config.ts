...
/**
 * X の重み付き文字数上限（全角2・半角1）。X API v2 の POST /2/tweets は
 * Premium でも280が上限（2026-09-02 に公式ドキュメント/開発者フォーラムで再確認）。
 * 長文投稿(最大25000)は Web/アプリのコンポーザ限定で、API からは作成できない。
 * もし将来 API が長文対応したら環境変数 MAX_TWEET_WEIGHT で引き上げる。
 */
